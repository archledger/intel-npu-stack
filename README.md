...
The current implementation is read-only. It defines strict platform profiles and provides bounded `status` discovery plus crash-isolated `doctor` inference without installing packages or changing system configuration. Fedora candidate packaging is under validation; installation remains a later reviewed phase. See [Runtime status and doctor](docs/runtime-doctor.md) for the diagnostic boundary and [candidate tools packaging](packaging/fedora/44/rpm/intel-npu-stack/README.md) for the package contract.
