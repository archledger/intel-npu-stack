...
with tempfile.TemporaryDirectory(prefix='profile-rpm-contract-') as temporary:
    ...
    for name,p in sorted(packages.items()):
        ...
        if name.startswith('libopenvino-') or name in ['intel-npu-compiler','openvino-plugins']:
            spec.append('Requires: openvino(x86-64) = 2026.2.0-2.intelnpu.fc44')
    ...
    assert profile['components']['level_zero_loader']['provider']['version']=='0:1.32.0-1.intelnpu.fc44'
    assert profile['components']['npu_userspace_driver']['provider']['version']=='0:1.38.0-1.intelnpu.fc44'
