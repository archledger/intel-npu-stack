...
| Tools and profile | Tools use the accepted provider SDK and manifest; the candidate records final provider and critical-file digests without a self-hash cycle. | Canonical fourteen-RPM candidate generation passed with tools release 3. Provider profile bytes stayed unchanged. The committed profile is qualified for release 0.1.0 with the kernel window `[7.2.5, 7.3.0)`; test candidates made with `retarget-candidate` stay unqualified, and signing does not promote them. |
